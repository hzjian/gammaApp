...
public interface TlGammaLayerLineRepository  extends PagingAndSortingRepository<TlGammaLayerLine,String>{
	...
	@Query("select a from TlGammaLayerLine a  where a.kernelClassid = ?1 and intersects( a.kernelGeom , ?2) = true") 
	public List<TlGammaLayerLine> getDataByFilter(String kernelClassid,Geometry ptGeom,Pageable pageable);
	
	@Query("select count(*) from TlGammaLayerLine a  where a.kernelClassid = ?1 and intersects( a.kernelGeom , ?2) = true") 
	...
	public List<String> getKernerIdByGeoFilter(String kernelClassid,Geometry geom);
